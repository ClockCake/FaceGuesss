...
//        selectedIndex = 0
        ...
        let titles = ["首页","工作台", "我的","脸猜"]

        let vcs = [ HomeViewController(title: "",isShowBack: false),
                    ActivityViewController(title: "",isShowBack: false),
                    RankViewController(title: "", isShowBack: false, url: "https://www.jd.com"),
                    PersonalViewController(title: "",isShowBack: false)
        ]
        for i in 0..<titles.count {
            let nav = UINavigationController(rootViewController: vcs[i])
           
            nav.tabBarItem.title = titles[i]

            
            nav.tabBarItem.setTitleTextAttributes([.foregroundColor:UIColor.red, .font:UIFont.systemFont(ofSize: 10)], for: .normal)
            nav.tabBarItem.setTitleTextAttributes([.foregroundColor:UIColor.green, .font:UIFont.systemFont(ofSize: 16)], for: .selected)
            self.addChild(nav)
        }
        if #available(iOS 13.0, *){
            let standardAppearance = self.tabBar.standardAppearance.copy()
            let inlineLayoutAppearance = UITabBarItemAppearance.init()
            inlineLayoutAppearance.normal.titleTextAttributes = [NSAttributedString.Key.foregroundColor :UIColor.red ,NSAttributedString.Key.font:UIFont.systemFont(ofSize: 10)]
            inlineLayoutAppearance.selected.titleTextAttributes = [NSAttributedString.Key.foregroundColor :UIColor.green ,NSAttributedString.Key.font:UIFont.systemFont(ofSize: 16)]
